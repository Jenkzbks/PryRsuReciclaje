...
<div class="card">
    <form method="POST" action="{{ route('admin.schedulings.store') }}">
        @csrf
        {{-- Alertas: errores y mensajes de sesión --}}
        @if(session('success'))
            ...
            <div class="row">
                ...
                <div class="col-md-4 mb-3">
                    <div class="card shadow">
                        ...
                        <div class="card-body">
                            {{-- Hidden flag to mark temporary removal from view --}}
                            <input type="hidden" name="group_{{ $group->id }}[removed]" value="0" class="group-removed-input" data-group-id="{{ $group->id }}">

                            {{-- Overlay shown when group is trashed (temporary removal) --}}
                            <div class="trashed-overlay" style="display:none;">
                                <div class="text"><strong>Grupo eliminado (temporal)</strong></div>
                                <button class="btn btn-sm btn-secondary btn-undo-trash" data-group-id="{{ $group->id }}">Deshacer</button>
                            </div>
                            ...
                            <p><strong>Zona:</strong> {{ $group->zone->name ?? '-' }}</p>
                            <p><strong>Turno:</strong> {{ $group->shift->name ?? '-' }}</p>
                            <p><strong>Días:</strong> {{ $group->days ?? '-' }}</p>
                            <p>
                                <strong>Vehículo:</strong> {{ $group->vehicle->plate ?? '-' }}
                                @if(optional($group->vehicle)->capacity)
                                    <span class="badge badge-warning">Capacidad: {{ $group->vehicle->capacity }}</span>
                                @endif
                            </p>

                            <div class="form-group">
                                <label>Conductor:</label>
                                <select name="group_{{ $group->id }}[driver]" class="form-control">
                                    <option value="">Seleccione</option>
                                    @foreach($drivers as $emp)
                                        <option value="{{ $emp->id }}" {{ $emp->id == $driverId ? 'selected' : '' }}>{{ $emp->lastnames }} {{ $emp->names }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="group_{{ $group->id }}[assistant1]" class="form-control">
                                    <option value="">Seleccione</option>
                                    @foreach($assistants as $emp)
                                        <option value="{{ $emp->id }}" {{ $emp->id == $assistant1Id ? 'selected' : '' }}>{{ $emp->lastnames }} {{ $emp->names }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="group_{{ $group->id }}[assistant2]" class="form-control">
                                    <option value="">Seleccione</option>
                                    @foreach($assistants as $emp)
                                        <option value="{{ $emp->id }}" {{ $emp->id == $assistant2Id ? 'selected' : '' }}>{{ $emp->lastnames }} {{ $emp->names }}</option>
                                    @endforeach
                                </select>
                            </div>
