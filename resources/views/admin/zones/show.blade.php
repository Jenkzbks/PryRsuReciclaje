...
<div class="row">
                ...
                @if($zone->hasPolygon())
                    ...
                    <script>
                        (function() {
                            ...
                            const coords = @json($zone->polygon_coordinates);
    ...
    <div class="col-md-6">
        ...
        <div class="card shadow-sm mb-4">
            ...
            <div class="card-body">
                <div class="row mb-3"><div class="col-4"><strong>Código:</strong></div><div class="col-8"><span class="badge badge-primary">{{ $zone->code }}</span></div></div>
                <div class="row mb-3"><div class="col-4"><strong>Nombre:</strong></div><div class="col-8">{{ $zone->name }}</div></div>
                <div class="row mb-3"><div class="col-4"><strong>Departamento:</strong></div><div class="col-8">{{ $zone->department?->name ?? 'No asignado' }}</div></div>
                ...
                <div class="row mb-3"><div class="col-4"><strong>Distrito:</strong></div><div class="col-8">{{ $zone->district?->name ?? 'Toda la provincia' }}</div></div>
                <div class="row mb-3"><div class="col-4"><strong>Descripción:</strong></div><div class="col-8">{{ $zone->description ?: 'Sin descripción' }}</div></div>
                ...
                @if($zone->hasPolygon())
                    <div class="row mb-3"><div class="col-4"><strong># Vértices:</strong></div><div class="col-8">{{ count($zone->polygon_coordinates) }}</div></div>
                    <div class="row mb-3"><div class="col-4"><strong>Área (km²):</strong></div><div class="col-8">{{ number_format($zone->area, 2) }} km²</div></div>
                @endif
                <div class="row mb-3"><div class="col-4"><strong>Creado:</strong></div><div class="col-8">{{ $zone->created_at->format('d/m/Y H:i') }}</div></div>
                <div class="row"><div class="col-4"><strong>Actualizado:</strong></div><div class="col-8">{{ $zone->updated_at->format('d/m/Y H:i') }}</div></div>
            </div>
        </div>
    </div>

    <div class="col-md-6">
        @if($zone->zonecoords->count() > 0)
        <div class="card shadow-sm mb-4">
            <div class="card-header bg-light">
                <h5 class="card-title mb-0">
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table mb-0 table-striped">
                        ...
                        <tbody>
                            @foreach($zone->zonecoords as $index => $coord)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
        ...
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                <div class="d-grid gap-2 d-md-flex justify-content-md-center flex-wrap">
                    <a href="{{ route('admin.zones.edit', $zone) }}" class="btn btn-primary m-2">
                        <i class="fas fa-edit"></i> Editar Zona
                    </a>

                    @if(!$zone->hasPolygon())
                        <a href="{{ route('admin.zones.edit', $zone) }}" class="btn btn-warning m-2">
...
{{-- Solo carga assets extra si NO es AJAX (acceso directo por URL) --}}
@section('css')
    @if(!request()->ajax())
